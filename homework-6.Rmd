...
arrange(collect_metrics(tune_res2), desc(mean))[4:9][1,])

all_round_best <- data.frame(all_round_best)

Method <- c("Pruned tree", "Random forest")

all_round_best['Method'] <- Method

all_round_best

absolute_best = select_best(all_round_best$mean)

best_final <- finalize_workflow(class_tree_wf, best_complexity)
...
augment(best_final_fit, new_data = Pokémon_test) %>%
  conf_mat(truth = type_1, estimate = c(.pred_Fire, .pred_Bug, .pred_Grass, .pred_Normal, .pred_Water, .pred_Psychic))
...
best_abalone = arrange(collect_metrics(abalone_res))[1,][1,]
best_abalone

best_abalone = select_best(abalone_res)
